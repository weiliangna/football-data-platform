<template>
  <div class="sidebar-layer">
    <aside class="app-sidebar" :class="{ open }">
      ...
      <nav aria-label="主导航">
        <router-link v-for="item in navigation" :key="item.to" :to="item.to" :class="{ active: isActive(item) }" @click="$emit('close')">
      ...
      <section class="sidebar-status">
        ...
        <template v-else-if="platforms.length"><div v-for="item in platforms" :key="item.platform_id"><i :class="{ active: Number(item.enabled) === 1 }"></i><span>{{ item.name || ('平台 ' + item.platform_id) }}</span><small>{{ Number(item.enabled) === 1 ? '已配置' : '未配置' }}</small></div></template>
        <p v-else>暂无平台配置</p>
      </section>
