...
const REFRESH_MS=30000,route=useRoute(),matches=ref([]),selectedId=ref(""),news=ref({}),category=ref(""),matchesLoading=ref(true),matchesError=ref(""),newsLoading=ref(false),newsRefreshing=ref(false),newsError=ref("");let timer=null,requestSerial=0
...
function selectMatch(id){if(!id||id===selectedId.value)return;selectedId.value=id;category.value="";loadNews()}
function loadNews(silent=false){const id=selectedId.value;if(!id)return;const serial=++requestSerial;if(!silent&&!items.value.length)newsLoading.value=true;newsRefreshing.value=true;newsError.value="";axios.get(`/api/matches/${encodeURIComponent(id)}/news`,{timeout:25000}).then(response=>{if(serial!==requestSerial)return;news.value=response.data?.data||{}}).catch(error=>{if(serial!==requestSerial)return;newsError.value=error.response?.data?.msg||"比赛新闻暂时不可用"}).finally(()=>{if(serial===requestSerial){newsLoading.value=false;newsRefreshing.value=false}})}
...
onMounted(()=>{loadMatches();timer=setInterval(refreshPage,REFRESH_MS)});onBeforeUnmount(()=>{if(timer)clearInterval(timer);requestSerial+=1})
</script>
