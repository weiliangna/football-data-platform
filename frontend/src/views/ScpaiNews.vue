...
const REFRESH_MS=30000,route=useRoute(),matches=ref([]),selectedId=ref(""),news=ref({}),category=ref(""),matchesLoading=ref(true),matchesError=ref(""),newsLoading=ref(false),newsRefreshing=ref(false),newsError=ref("");let timer=null,requestSerial=0,newsController=null;const newsCache=new Map()
...
onMounted(()=>{loadMatches();timer=setInterval(refreshPage,REFRESH_MS)});onBeforeUnmount(()=>{if(timer)clearInterval(timer);newsController?.abort();requestSerial+=1})
</script>
