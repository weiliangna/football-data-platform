...
function selectMatch(id){if(!id||id===selectedId.value)return;selectedId.value=id;loadSelected()}
function loadSelected(silent=false){const id=selectedId.value;if(!id)return;const serial=++requestSerial;if(!silent)detailLoading.value=true;detailError.value="";axios.get(`/api/matches/${encodeURIComponent(id)}`,{timeout:25000}).then(response=>{if(serial!==requestSerial)return;applyDetail(response.data?.data||{})}).catch(error=>{if(serial!==requestSerial)return;detailError.value=error.response?.data?.msg||"盘口详情暂时不可用"}).finally(()=>{if(serial===requestSerial)detailLoading.value=false});loadContext(id,silent)}
function loadContext(id=selectedId.value,silent=false){if(!id)return;if(!silent)contextLoading.value=true;contextError.value="";const serial=requestSerial;axios.get(`/api/matches/${encodeURIComponent(id)}/context`,{timeout:25000}).then(response=>{if(serial!==requestSerial)return;context.value=response.data?.data||{}}).catch(error=>{if(serial!==requestSerial)return;context.value={};contextError.value=error.response?.data?.msg||"基本面暂时不可用"}).finally(()=>{if(serial===requestSerial)contextLoading.value=false})}
...
onMounted(()=>{loadDashboard();timer=setInterval(refreshPage,REFRESH_MS)});onBeforeUnmount(()=>{if(timer)clearInterval(timer);requestSerial+=1})
</script>
