...
function selectMatch(id){if(!id||id===selectedId.value)return;selectedId.value=id;const cached=detailCache.get(id);if(cached)applyDetail(cached);const cachedContext=contextCache.get(id);if(cachedContext)context.value=cachedContext.data||cachedContext;loadSelected(Boolean(cached))}
...
function scheduleRefresh(){if(timer)clearInterval(timer);timer=document.visibilityState==="visible"?setInterval(refreshPage,REFRESH_MS):null}
function handleVisibilityChange(){scheduleRefresh();if(document.visibilityState==="visible")refreshPage()}
onMounted(()=>{loadDashboard();document.addEventListener("visibilitychange",handleVisibilityChange);scheduleRefresh()});onBeforeUnmount(()=>{if(timer)clearInterval(timer);document.removeEventListener("visibilitychange",handleVisibilityChange);detailController?.abort();contextController?.abort();requestSerial+=1})
</script>
