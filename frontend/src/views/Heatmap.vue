<template>
  <section class="page-shell heatmap-page">
    <header class="heatmap-hero app-card">
      ...
      <button class="primary-button" type="button" @click="load">刷新热力图</button>
    </header>
...
function heatStyle(value) { const share = Math.max(0, Math.min(Number(value || 0), 100)); return { background: share ? `linear-gradient(90deg, rgba(217,255,53,.42) ${share}%, rgba(250,250,248,.75) ${share}%)` : "#fafaf8" } }
