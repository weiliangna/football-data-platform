...
import { loadAnalysis,loadDashboard,loadHeatmap,loadMatches,loadNews,loadPlans,loadResults,loadUsers } from './services/live.js';
...
document.addEventListener('click',e=>{const t=(e.target as Element).closest<HTMLElement>('[data-action],[data-filter],[data-segment],[data-open-match],[data-open-plan],[data-open-user],[data-follow],[data-page],[data-heat-play],[data-ranking-tab],[data-global-action]');if(!t)return;
 ...
 if(t.dataset.openMatch){if(route()==='matches')matches.selectedMatch=t.dataset.openMatch;else dashboard.selectedMatch=t.dataset.openMatch;render();return}if(t.dataset.openPlan){plans.selectedPlan=t.dataset.openPlan;render();return}if(t.dataset.openUser){if(route()==='ranking')ranking.selectedUser=t.dataset.openUser;else usersState.selectedUser=t.dataset.openUser;render();return}if(t.dataset.follow){const id=t.dataset.follow;usersState.followed.has(id)?usersState.followed.delete(id):usersState.followed.add(id);render();toast(usersState.followed.has(id)?'已关注用户':'已取消关注');return}if(t.dataset.page){plans.page=Number(t.dataset.page);render();return}if(t.dataset.heatPlay){heat.play=t.dataset.heatPlay as BettingPlay;render();return}if(t.dataset.rankingTab){ranking.tab=t.dataset.rankingTab;render();return}
});
