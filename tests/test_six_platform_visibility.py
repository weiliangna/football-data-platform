...
class PlatformApiVisibilityTests(unittest.TestCase):
    def test_stopped_platforms_are_omitted_from_public_list(self):
    ...
    def test_unrelated_database_platforms_are_not_public(self):
        module = load_platform_module()
        rows = [
            {
        ...
        self.assertNotIn("好店主", by_name)
        self.assertNotIn("启示录", by_name)
        self.assertNotIn("既有平台", by_name)

    def test_database_failure_returns_safe_active_platform_fallback(self):
        def unavailable():
...
class FrontendPlatformVisibilityTests(unittest.TestCase):
    def test_experts_page_uses_only_active_platform_fallbacks(self):
        source = (
